classdef KineMod %KinematicModel 
    ...
    methods (Static) %function that cannot change the variable properties (state) of
        ...
        function pos = IK_MAU(C_coord,verbose)
            ...
            pos = ArmPos();
            pos.C = C_coord;
            [r1, error1] = KineMod.CtoA(pos,verbose);
            if error1
                if verbose 
                    disp('failed to find the A coordinate');
                end
                return
            end
            [r2, error2] = KineMod.BtoE(r1,verbose);
            if error2
                if verbose 
                    disp('failed to find the E coordinate');
                end
                return 
            end
        ...
        function [finalPos, error] = CtoA(CurPos, verbose)
            ...
            error = false;
            A_pos = CircCirc(CurPos.C,CurPos.Z,CurPos.L6+CurPos.L7,CurPos.L5,verbose); %find possible positions for A
            ...
            for i=1:length(A_pos) %itterating over position array
                ...
                if pos(i).checkPhiZ(verbose) %if phiZ is valid will return this solution
            ...
            if j == 1 %no positions are found
                if verbose 
                    disp("CtoA: No valid positions found") ;
                end
                finalPos = [];
                error = true;
            end
        ...
        function [finalPos, error]= BtoE(positions,verbose)
            ...
            for i=1:L %itterating over the input positions
                ...
                E_pos = CircCirc(CurPos.X,CurPos.B,CurPos.L3,CurPos.L4,verbose); %finding possible positions for E
                pos = [CurPos; CurPos]; %creating position array
            ...
            if k == 1
                if verbose
                   disp("BtoE: no valid positions found"); 
                end
                finalPos = [];
                error = true;
            end
